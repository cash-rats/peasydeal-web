...
Low-effort, maintenance-focused cleanups for the checkout flow:

- **Order UUID handling**: `route.tsx` branches on `state.orderUUID`, but only `useCreateOrder` tracks the UUID. Either persist the UUID into reducer state or remove the unused branch to avoid dead code and confusion.
- **Form state updates**: `handleFormChange` only updates shipping fields while contact updates happen via `ContactInfoForm` callbacks. Add a brief note or consolidate state updates so the split responsibility is explicit.
- **Phone validation**: Move `validatePhone` into a small helper (e.g., `routes/checkout/utils.ts`) to declutter the component and enable reuse/tests.
- **Action payload parsing**: Centralize JSON parsing for forms/promo codes in helpers to avoid repeated try/catch blocks and keep action handlers linear.
