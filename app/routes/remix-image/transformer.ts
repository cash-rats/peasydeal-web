...
type CustomTransformer = {
  ...
  transform: (input: { url: string; data: Uint8Array; contentType: MimeType }, output: Required<TransformOptions>) => Promise<Uint8Array>;
};
...
const transformer: CustomTransformer = {
  ...
  transform: async ({ url, data }, { contentType: outputContentType, width, height, fit, position, background, quality, compressionLevel, loop, delay, blurRadius, rotate, flip, crop }) => {
