...
import { BlogPostCard, BlogPagination, BlogSidebar } from '~/components/v2/BlogLayout';
...
export const loader = async ({ params }: { params: Record<string, string | undefined> }) => {
  try {
    ...
    return Response.json({
      ...
      latestPosts: latest,
      canonicalLink: `${getCanonicalDomain()}/blog/page/${pageNum}`,
    });
  } catch (e) {
    console.error(e);
...
export default function BlogPage() {
  ...
  const filteredPosts = useMemo(() => {
    if (!activeTag) return postSummaries;
    return postSummaries.filter((post: any) =>
      (post.tags ?? []).includes(activeTag)
    );
  }, [postSummaries, activeTag]);
    ...
    <div className="v2 max-w-[var(--container-max)] mx-auto px-4 redesign-sm:px-6 redesign-md:px-12 py-6">
      ...
      <div className="flex gap-12">
        ...
        <div className="flex-1 min-w-0">
          ...
          {/* Pagination — only when showing all posts */}
          {!activeTag && (
            <BlogPagination
