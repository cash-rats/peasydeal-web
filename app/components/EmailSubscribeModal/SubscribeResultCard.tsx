...
interface SubscribeResultCardProps {
  error: ApiErrorResponse | null;
  onClose: () => void;
  showHint?: boolean;
}

export default function SubscribeResultCard({
  error,
  onClose,
  showHint = true,
}: SubscribeResultCardProps) {
  return (
    <div className="font-poppins">
      <div className="relative overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-[0_10px_40px_-10px_rgba(0,0,0,0.08)]">
        <button
          type="button"
        ...
        <div className="border-t border-slate-100 bg-slate-50 px-8 py-5">
          ...
          {!error && showHint ? (
            <p className="mt-4 text-center text-xs text-slate-500 opacity-60">
              Didn’t receive the email?{' '}
              <span className="underline decoration-slate-400/60 underline-offset-2 hover:text-primary">
                Check spam
              </span>{' '}
              or{' '}
              <span className="underline decoration-slate-400/60 underline-offset-2 hover:text-primary">
