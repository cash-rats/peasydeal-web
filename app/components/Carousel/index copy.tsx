	...
	return (
		<>
			...
			<div className="product-carousel-container">
										...
										images.map((image, index) => {
											return (
												<div className="flex" key={index}>
													<LazyLoadImage
														wrapperClassName={'flex aspect-square w-full h-full items-center m-4'}
														alt={title}
														src={image}
														placeholder={<div className='block w-full h-full bg-[#efefef] animate-pulse aspect-square'/>}
													/>
												</div>
											)
										})
				...
				<div className='bg-slate-100 w-full h-[24px] flex justify-center items-center'>
					<small className='font-bold'>{`${activeSlide < 0 ? 1 : activeSlide + 1}/${images.length || 0}`}</small>
				</div>
